...
/**
 * This function creates the JFrame for the sleeping pill medication 
 * 
 * @author Rachel Lee
 *
 */
public class SleepingPillMedicationPage extends JFrame implements ActionListener{
	...
	public SleepingPillMedicationPage(Point dim, JFrame searchMedicationFrame){
	...
	private JPanel informationArea(){
		...
		ImageIcon pic = new ImageIcon(getClass().getResource("medications/sleepingPills.png"));
		
		//intialise all the JLabels
		JLabel medicationPic = new JLabel(pic,JLabel.CENTER);
		JLabel ingredientsText = new JLabel(" Medication Ingredients : 25 mg Diphenhydramine hydrochloride ", JLabel.CENTER);
		JLabel safeDosage = new JLabel("Approved safe dosage: 1000 units/ml",JLabel.CENTER);
		JLabel mostUse = new JLabel("Mostly used for: Insomnia", JLabel.CENTER);
		JLabel sideEffects = new JLabel("Affected by these medication: ",JLabel.CENTER);
		
		//initlaise the button to display the medication side effects with other medication
		JButton affectedDrugs = new JButton ("AntiBiotics");
	...
	public void actionPerformed(ActionEvent e) {
		...
		//this will check if the button that display the side effects of each medication when it comes in contact with the current medication is clicked
			...
			JOptionPane.showMessageDialog(null, "Possible side effects: dizzines, drowsiness, dry mouth,throat,and nose, thickening of mucus in nose or throat."
