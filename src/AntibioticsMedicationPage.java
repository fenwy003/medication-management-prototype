...
/**
 * This function creates the JFrame for the AntiBiotics medication.
 * @author Rachel Lee
 *
 */
public class AntibioticsMedicationPage extends JFrame implements ActionListener{
	...
	public AntibioticsMedicationPage(Point dim, JFrame searchMedicationFrame){
	...
	private JPanel informationArea(){
		...
		ImageIcon pic = new ImageIcon(getClass().getResource("medications/antibiotics.png"));
		
		//intialise all the JLabels
		JLabel medicationPic = new JLabel(pic,JLabel.CENTER);
		JLabel ingredientsText = new JLabel(" Medication Ingredients :  bacitracin, chloramphenicol, gentamicin sulfate", JLabel.CENTER);
		JLabel safeDosage = new JLabel("Approved safe dosage: 775 units/ml",JLabel.CENTER);
		JLabel mostUse = new JLabel("Mostly used for: Treating bacterial infections", JLabel.CENTER);
		...
		JButton affectedDrugs = new JButton ("Insulin");
	...
	public void actionPerformed(ActionEvent e) {
		...
		//this will check if the button that display the side effects of each medication when it comes in contact with the current medication is clicked
			...
			JOptionPane.showMessageDialog(null, "Possible side effects: vommiting and fainting spells"
